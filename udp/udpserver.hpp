...
//typename std::function<string(const string&)> func;
...
public:
    ...
    void Init()
    {
        ...
        local.sin_addr.s_addr = inet_addr((_ip.c_str())); //inet_addr将点分十进制（如"113.45.157.139"）的ipv4地址字符串转换为32位网络字节序二进制数值
        //bind失败返回-1，设置错误信息
    ...
    void Run()
    {
        ...
        while (_isrunning)
        {
            ...
            buffer[n] = 0;
            string temp = buffer;
            cout << "server echo# " << temp.c_str() << endl;
            string echostring(buffer);
